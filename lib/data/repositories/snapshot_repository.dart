...
class SnapshotRepository {
  ...
  Future<Map<String, dynamic>> exportSnapshot() async {
    ...
    final poRows = await (db.select(db.purchaseOrders)
    ...
    final settingsData =
        await (db.select(db.settingsRow)..where((t) => t.id.equals(0)))
            .getSingleOrNull();
    final Map<String, dynamic>? settings = settingsData == null
        ...
        : <String, dynamic>{
            ...
            if (settingsData.branchNo != null) 'branchNo': settingsData.branchNo,
          };
    ...
    final quoteRows = await (db.select(db.quotes)
          ..orderBy([(t) => OrderingTerm.desc(t.date)]))
        .get();
    ...
    final returnRows = await (db.select(db.returns)
          ..orderBy([(t) => OrderingTerm.desc(t.date)]))
        .get();
    ...
    final movementRows = await (db.select(db.movements)
          ..orderBy([(t) => OrderingTerm.desc(t.date)]))
        .get();
    ...
    final cpRows = await (db.select(db.creditPayments)
          ..orderBy([(t) => OrderingTerm.desc(t.date)]))
        .get();
    ...
    Map<String, dynamic> shiftToJson(ShiftRow s) {
      final entries = (entriesByShift[s.id] ?? const <DrawerEntryRow>[])
          .toList()
    ...
    final parkedRows = await (db.select(db.parkedSales)
          ..orderBy([(t) => OrderingTerm.desc(t.parkedAt)]))
        .get();
    ...
    final schemaMeta =
        await (db.select(db.appMeta)..where((t) => t.key.equals('schema_version')))
            .getSingleOrNull();
    ...
    await db.transaction(() async {
      ...
      for (final s in asList(data['sa_sales'])) {
        await db.into(db.sales).insert(SalesCompanion.insert(
              ...
              mechanicDelta: Value(
                  s['mechanicDelta'] == null ? null : _asDouble(s['mechanicDelta'])),
      ...
      for (final q in asList(data['sa_quotes'])) {
        await db.into(db.quotes).insert(QuotesCompanion.insert(
              ...
              subtotal: Value(q['subtotal'] == null ? null : _asDouble(q['subtotal'])),
              discount: Value(q['discount'] == null ? null : _asDouble(q['discount'])),
              ...
              validDays: Value(q['validDays'] == null ? null : _asInt(q['validDays'])),
            ));
      ...
      for (final r in asList(data['sa_returns'])) {
        ...
        for (final it in asList(r['items'])) {
          await db.into(db.returnItems).insert(ReturnItemsCompanion.insert(
                ...
                price: _asDouble(it['price']),
                originalQty: Value(
                    it['originalQty'] == null ? null : _asInt(it['originalQty'])),
              ));
        }
      }
      ...
      if (cd is Map) {
        ...
        final shiftId = await db.into(db.shifts).insert(ShiftsCompanion.insert(
              ...
              physicalCash: Value(
                  shift['physicalCash'] == null ? null : _asDouble(shift['physicalCash'])),
        ...
        for (final e in asList(shift['entries'])) {
          await db.into(db.drawerEntries).insert(DrawerEntriesCompanion.insert(
      ...
      for (final p in asList(data['sa_parked'])) {
        ...
        await db.into(db.parkedSales).insert(ParkedSalesCompanion.insert(
              id: id,
              parkedAt: parkedAt,
