...
void main() {
  ...
  test('insufficient stock throws Thai message and leaves data unchanged',
      () async {
    ...
    final input = SaleInput(
      ...
      items: const [
        SaleLineInput(
            productId: 'p8', name: 'Piston Kit STD', qty: 6, price: 3200),
      ],
    );
    ...
    await expectLater(
      repo.saveSale(input),
      throwsA(predicate(
          (e) => e.toString().contains('สต็อกไม่พอ:\n') &&
  ...
  test('good sale decrements stock exactly and sets points/receiptNo',
      () async {
    ...
    final input = SaleInput(
      ...
      items: const [
        SaleLineInput(
            productId: 'p1', name: 'Oil Filter', qty: 3, price: 85, partNo: 'HN-15412-KVB'),
      ],
    );
  ...
  test('customer sale increases points + totalSpend', () async {
    ...
    final input = SaleInput(
      ...
      items: const [
        SaleLineInput(productId: 'p2', name: 'Spark Plug NGK', qty: 1, price: 120),
      ],
    );
    final sale = await repo.saveSale(input);
  ...
  test('mechanic credit sale increases creditBalance by total', () async {
    final mBefore =
        await (db.select(db.mechanics)..where((t) => t.id.equals('m1')))
            .getSingle();
    ...
    final input = SaleInput(
      ...
      items: const [
        SaleLineInput(
            productId: 'p3', name: 'Drive Chain #520', qty: 1, price: 350),
      ],
    );
  ...
  test('mechanic non-credit sale with markup: no creditBalance, markup added',
      () async {
    final input = SaleInput(
      subtotal: 350,
      discount: 0,
    ...
    final m =
        await (db.select(db.mechanics)..where((t) => t.id.equals('m2')))
            .getSingle();
  ...
  test('getSales returns newest first', () async {
    Future<void> sell(String productId, double total) => repo.saveSale(SaleInput(
          subtotal: total,
          discount: 0,
          total: total,
          paymentMethod: 'เงินสด',
  ...
  test('getRefundedQty sums ReturnItems across returns for a sale', () async {
    final sale = await repo.saveSale(SaleInput(
      subtotal: 170,
      discount: 0,
      total: 170,
      paymentMethod: 'เงินสด',
    ...
    await db.into(db.returns).insert(ReturnRow(
          id: 'r1',
          cnNo: 'CN1',
          saleId: sale.id,
