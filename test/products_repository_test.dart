...
void main() {
  ...
  test('add with a fresh partNo inserts and assigns a p-prefixed id', () async {
    final added = await repo.add(ProductsCompanion.insert(
      id: 'IGNORED',
      partNo: 'NEW-001',
      name: 'New Part',
      nameTH: 'ของใหม่',
  ...
  test('update to a colliding partNo (another product) returns false', () async {
  ...
  test('update keeping the same partNo on the same product succeeds', () async {
    final ok = await repo.update(
      'p1',
      const ProductsCompanion(partNo: Value('HN-15412-KVB'), name: Value('Renamed')),
    );
    expect(ok, isTrue);
  ...
  test('update with a non-colliding partNo succeeds and applies patch', () async {
    final ok = await repo.update(
      'p1',
  ...
  test('adjustStock below zero CLAMPS to 0 and writes a movement with stockAfter 0',
      () async {
  ...
  test('catColor is stable by category index (palette order)', () async {
    ...
    for (var i = 0; i < cats.length; i++) {
      ...
      expect(color, ProductsRepository.catPalette[i % ProductsRepository.catPalette.length]);
    }
  ...
  test('catColor for an unknown category uses the hash fallback (deterministic)',
      () async {
