...
void main() {
  ...
  test('saveQuote sets quoteNo + open + validUntil and writes NO product/sale rows',
      () async {
    ...
    final saved = await repo.saveQuote(
      QuoteInput(
        ...
        validDays: 30,
        items: const [
          QuoteLineInput(productId: 'p1', name: 'น้ำมันเครื่อง', qty: 2, price: 50),
        ],
      ),
    );
  ...
  test('saveQuote default status is open and falls back to settings quoteValidDays',
      () async {
    final saved = await repo.saveQuote(
      const QuoteInput(items: [
        QuoteLineInput(name: 'item', qty: 1, price: 10),
      ]),
    );
    expect(saved.status, 'open');
  ...
  test('duplicateQuote produces an open copy with a new id/quoteNo, same items',
      () async {
    ...
    expect(dup.status, 'open'); // forced open even though source was converted
  ...
  test('purgeOldQuotes removes an expired-long-ago quote and keeps a fresh one',
      () async {
    ...
    final staleValidUntil = DateTime.now().subtract(const Duration(days: 200));
    ...
    final staleItems = await (db.select(db.quoteItems)
          ..where((t) => t.quoteId.equals('q_stale')))
        .get();
    expect(staleItems, isEmpty);
  });

  test('purgeOldQuotes keeps a converted quote whose convertedAt is recent',
      () async {
