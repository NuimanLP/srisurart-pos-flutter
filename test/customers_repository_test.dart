...
void main() {
  ...
  test('addCustomer auto-increments CUS code from CUS001 on an empty table',
      () async {
    await clearCustomers();
    final c = await repo.addCustomer(
      const CustomersCompanion(name: Value('Walk-in'), nameTH: Value('ลูกค้า')),
    );
    expect(c.code, 'CUS001');
  });

  test('addCustomer zero-inits points and totalSpend and sets createdAt/id',
      () async {
    await clearCustomers();
    final c = await repo.addCustomer(
  ...
  test('addCustomer auto-increments past the existing max CUS number',
      () async {
    await clearCustomers();
  ...
  test('addCustomer overrides caller-supplied id/code/points/totalSpend',
      () async {
    await clearCustomers();
    final c = await repo.addCustomer(
  ...
  test('updateCustomer merges patch fields only', () async {
    ...
    final updated =
        await (db.select(db.customers)..where((t) => t.id.equals(c.id)))
            .getSingle();
