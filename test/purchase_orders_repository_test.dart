...
void main() {
  ...
  test(
    'savePO assigns id (po…), poNo (PO…), status open, and writes items',
    () async {
  ...
  test(
    'receivePO returns unmatched partNo and leaves stock untouched for it',
    () async {
      await insertProduct(id: 'tp1', partNo: 'TEST-1', stock: 10, cost: 100);
      ...
      final ghost = await (db.select(
        db.products,
      )..where((t) => t.partNo.equals('NOPE-9'))).get();
  ...
  test('receivePO marks the PO received and sets receivedAt', () async {
    ...
    final updated = await (db.select(
      db.purchaseOrders,
    )..where((t) => t.id.equals(po.id))).getSingle();
    expect(updated.status, 'received');
    expect(updated.receivedAt, isNotNull);
  });

  test(
    'receivePO writes a receive movement row with stockAfter and note',
    () async {
      await insertProduct(id: 'tp1', partNo: 'TEST-1', stock: 10, cost: 100);
  ...
  test(
    'receivePO uses newCost=item.cost when cost>0; falls back to oldCost',
    () async {
      ...
      final po = await repo.savePO(
        const PoInput(
          supplier: 'Acme',
          items: [
            PoLineInput(partNo: 'TEST-1', name: 'Widget', qty: 5, cost: 0),
          ],
        ),
      );
  ...
  test('getPOs returns newest first', () async {
    final first = await repo.savePO(
      const PoInput(
        supplier: 'A',
        items: [PoLineInput(partNo: 'X', name: 'x', qty: 1, cost: 1)],
      ),
    );
    final second = await repo.savePO(
      const PoInput(
        supplier: 'B',
