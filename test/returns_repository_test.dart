...
void main() {
  ...
  test('Sale not found throws', () async {
    expect(
      () => repo.createReturn(
        const ReturnInput(saleId: 'nope', items: [], refundMethod: 'เงินสด'),
      ),
  ...
  test('item not in bill throws "ไม่อยู่ในบิลนี้"', () async {
    ...
    expect(
      ...
      throwsA(
        predicate((e) => e.toString().contains('Spark Plug: ไม่อยู่ในบิลนี้')),
      ),
    );
  });
  ...
  test(
    ...
    () async {
      ...
      final stockBefore = (await (db.select(
        db.products,
      )..where((p) => p.id.equals('p1'))).getSingle()).stock;
      final custBefore = await (db.select(
        db.customers,
      ...
      final ret = await repo.createReturn(
        const ReturnInput(
          saleId: 's_partial',
          items: [
            ReturnLineInput(
      ...
      // Stock restored +1.
      final stockAfter = (await (db.select(
        db.products,
      )..where((p) => p.id.equals('p1'))).getSingle()).stock;
      expect(stockAfter, stockBefore + 1);

      // Customer spend −85, points −8.
      final custAfter = await (db.select(
        db.customers,
      )..where((c) => c.id.equals('c1'))).getSingle();
      ...
      final sale = await (db.select(
        db.sales,
      )..where((s) => s.id.equals('s_partial'))).getSingle();
  ...
  test('full return auto-voids the sale', () async {
    ...
    var sale = await (db.select(
      db.sales,
    )..where((s) => s.id.equals('s_full'))).getSingle();
  ...
  test(
    'refundMethod "หักจากเครดิต" reduces mechanic creditBalance; cash does NOT',
    () async {
      ...
      var mech = await (db.select(
        db.mechanics,
      )..where((m) => m.id.equals('m1'))).getSingle();
      expect(
        mech.creditBalance,
        500,
        reason: 'cash refund must NOT touch credit balance',
      );
  ...
  test('getReturns returns newest first', () async {
    ...
    final r1 = await repo.createReturn(
      const ReturnInput(
        saleId: 's_order',
        items: [
          ReturnLineInput(
    ...
    final r2 = await repo.createReturn(
      const ReturnInput(
        saleId: 's_order',
        items: [
          ReturnLineInput(
