...
void main() {
  ...
  test('addMechanic auto-increments the M-code (max+1, padded to 3)', () async {
    final m = await repo.addMechanic(MechanicsCompanion.insert(
      id: 'IGNORED',
    ...
    final m2 = await repo.addMechanic(MechanicsCompanion.insert(
      id: 'IGNORED',
      code: 'x',
      name: 'Another',
      createdAt: 'x',
  ...
  test('addCreditPayment records the payment and reduces creditBalance',
      () async {
    // Give m1 a balance of 1000.
    await (db.update(db.mechanics)..where((t) => t.id.equals('m1')))
        .write(const MechanicsCompanion(creditBalance: Value(1000)));
  ...
  test('addCreditPayment clamps creditBalance at 0 (never negative)', () async {
    await (db.update(db.mechanics)..where((t) => t.id.equals('m1')))
        .write(const MechanicsCompanion(creditBalance: Value(200)));
