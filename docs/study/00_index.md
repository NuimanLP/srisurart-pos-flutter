...
3. **ยุค 3 — multi-tenant + server + CI/CD (ตอนนี้)**: ตั้งแต่ 2026-09-04 เป็นต้นมา `main` กลาย
   ...
   **CI/CD** (ระบบทดสอบ+ส่งโค้ดขึ้น production อัตโนมัติ — อธิบายละเอียดใน 07/08) กติกาการขาย/คืน/
...
- **Phase 2** (กำลังทำ) = เอาความสามารถ offline ของยุค 2 กลับมาใส่ในระบบยุค 3 — ใช้กลไกที่เรียกว่า
  **outbox** (คิวเก็บงานที่ยังส่งไม่สำเร็จ รอส่งซ้ำภายหลัง — อธิบายละเอียดในบท 06)
...
ชุดเอกสารนี้มี 9 บท (00–08) แต่ละบทตอบคำถามคนละมุมของระบบ

| บท | ชื่อ | ตอบคำถามอะไร | เวลาอ่าน | ต้องอ่านมาก่อน |
|---|---|---|---|---|
| **00** | แผนที่ + พื้นฐาน (บทนี้) | โปรเจกต์นี้คืออะไร, ศัพท์พื้นฐานที่ต้องรู้ก่อน | 60–90 นาที | ไม่มี |
| **01** | System Architecture | ชิ้นส่วนทั้งหมด (Flutter, NestJS, Postgres, Redis, Nginx…) เชื่อมกันยังไง | 40–60 นาที | 00 |
| **02** | Use Case | ใครใช้ระบบบ้าง (เจ้าของร้าน, พนักงาน, เครื่อง POS, เครื่อง backoffice, platform admin) ทำอะไรได้บ้าง | 30–40 นาที | 00, 01 |
| **03** | Frontend | แอป Flutter ทำงานยังไง, เทคโนโลยีอะไรบ้าง | 50–70 นาที | 00, 01 |
| **04** | Backend | server NestJS ทำงานยังไง, tenant/transaction/idempotency คืออะไร | 60–80 นาที | 00, 01 |
| **05** | Database | ทำไมต้องมี database, RLS คืออะไร, ทำไมใช้ Postgres/Drift/Redis/etcd | 60–80 นาที | 00, 01, 04 |
| **06** | Offline & Phase 2 | จาก offline-first (POC) ไปสู่ outbox/sync ของ phase 2 | 40–60 นาที | 00, 01, 03, 04 |
| **07** | DevOps | Docker, Compose, Nginx, Ansible, Prometheus/Grafana, GHCR, Trivy คืออะไรและทำงานยังไง | 50–70 นาที | 00, 01, 04 |
| **08** | CI/CD เชิงลึก | ทุก stage ของ pipeline, ผลลัพธ์จริง, กรณี deploy ติด FortiGate | 40–60 นาที | 00, 07 |

### ลำดับการอ่าน (Mermaid)

```mermaid
flowchart TD
    A["00 แผนที่ + พื้นฐาน"] --> B["01 System Architecture"]
    B --> C["02 Use Case"]
    B --> D["03 Frontend"]
    B --> E["04 Backend"]
    E --> F["05 Database"]
    D --> G["06 Offline / Phase 2"]
    E --> G
    E --> H["07 DevOps"]
    H --> I["08 CI/CD เชิงลึก"]
```

### สองเส้นทางอ่าน

**เส้นทาง A — อยากรู้ภาพรวมเร็ว (ประมาณ 3 ชั่วโมง)**
`00 → 01 → 02 → 08` (อ่านหัวข้อสรุป/status ของแต่ละบทเป็นหลัก ข้าม "🔍 ของจริงใน repo" ที่ลงโค้ดลึก)

**เส้นทาง B — อยากเข้าใจลึกทั้งระบบ (ประมาณ 8–10 ชั่วโมง)**
`00 → 01 → 02 → 03 → 04 → 05 → 06 → 07 → 08` อ่านทุกส่วนรวม "🔍 ของจริงใน repo" และ "⚠️ บทเรียน
จากของจริง" — เหมาะกับคนที่จะเริ่มแก้โค้ดในนี้จริงๆ
...
เซิร์ฟเวอร์) เป็นอีก process หนึ่ง (จริงๆ รันพร้อมกัน 3 ชุด เรียกว่า `api-1/api-2/api-3` — ดูบท 01)
...
- **IP address** คือ "ที่อยู่" ของเครื่องหนึ่งบนเครือข่าย เหมือนบ้านเลขที่ เช่น `172.30.58.20` (ที่
  อยู่ของ VM ชื่อ `mob04` ที่ใช้รันระบบนี้จริง — จะเจอในบท 07/08)
...
- **Status code** — ตัวเลข 3 หลักที่ server ตอบกลับ บอกว่าสำเร็จหรือไม่
  ...
  - `5xx` = ฝั่ง server มีปัญหา (เช่น `500 Internal Server Error` = server พังเอง ไม่ใช่ความผิด
    client)
  - กติกาสำคัญของโปรเจกต์นี้ (จะเจอในบท 04): "มีแค่ `4xx` เท่านั้นที่ถือเป็นคำตอบที่แน่นอน — ส่วน
...
HTTPS พอร์ต 443 เท่านั้น (ดูบท 01/07)
...
PR แต่ละอันจะถูกสร้างเป็น branch ย่อยชั่วคราว แก้เสร็จ ผ่านการตรวจสอบและ CI (ดูบท 08) แล้วค่อย
merge เข้า `main`
...
Actions** (ระบบรันงานอัตโนมัติเมื่อมีการ push/PR — คือเครื่องมือ CI/CD ที่ใช้ในบท 07/08) repo นี้
...
```
frontend/                  แอป Flutter ทั้งหมด → บท 03
server/                    backend NestJS ทั้งหมด → บท 04
deploy/                    สคริปต์/config การ deploy (Ansible, Prometheus, Grafana) → บท 07
docs/                      เอกสารทุกชนิด (รวมโฟลเดอร์ study/ ที่คุณกำลังอ่านอยู่นี้)
  Backend_design/          สเปกผูกพันของ backend + adr/ (บันทึกการตัดสินใจ) → บท 04, 05
...
.github/workflows/         ไฟล์ config ของ GitHub Actions (flutter.yml, server.yml, deploy.yml) → บท 08
...
- ชุดเอกสารนี้มี 9 บท (00–08) อ่านตามลำดับ dependency ใน flowchart ด้านบน มี 2 เส้นทาง (เร็ว/ลึก)
...
บทถัดไป: [`01_architecture.md`](01_architecture.md) — ดูภาพรวมว่าชิ้นส่วนทั้งหมด (Flutter, NestJS,
PostgreSQL, Redis ×2, Nginx, etcd) เชื่อมกันเป็นระบบเดียวได้ยังไง
