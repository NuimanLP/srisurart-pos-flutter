...
- [x] ดับ `redis-cache` แล้วร้านที่ `suspended` **ยังถูกปฏิเสธ** และร้านปกติ**ยังใช้งานได้** (ADR-0003 ข้อ 5: status ตกไปอ่าน Postgres ไม่ fail-open/closed) — **#383 2026-09-22** (`server/test/redis-cache-outage.e2e-spec.ts`) ปิดช่องว่างที่ untick ไว้ก่อนหน้านี้ (เคยอ้าง `tenant-scope.e2e-spec.ts:186` ซึ่งแค่ `vi.spyOn` เมธอดของ cache client และยิง `/api/v1/tx4-probe` — ทั้งสองข้อนั้นถูกลบออกจากไฟล์นั้นในทิกเก็ตนี้แล้ว) เทสต์ใหม่ทำให้ `redis-cache` **เข้าไม่ถึงจริง** สองแบบ ไม่ mock เมธอดใดเลย: "refused" — `REDIS_CACHE_URL` ชี้ไปพอร์ตที่เปิดแล้วปิดทันที ทำให้ ECONNREFUSED จริง (`:202`) และ "hung" — TCP server จริงที่ตอบ handshake ของ ioredis จนถึง `ready` แล้วเงียบสนิท จำลองเคส "connect ค้าง" ของ #140 (`:273-274`) ทั้งสองสถานการณ์ยืนยันกับ `GET /products` (200, เห็นสินค้าที่ seed) + `POST /sales` (201, **stock ลดจริง 10 → 9** อ่านก่อน/หลัง) สำหรับร้าน active แล้วจึงสั่ง `suspended` ตรง Postgres และยืนยันคำขอถัดไปได้ `403 TENANT_SUSPENDED` ทันที ไม่ต้องรอ token หมดอายุ (`assertActiveSellsAndSuspendedIsRejectedAtOnce`, ใช้ร่วมกันทั้งสองสถานการณ์) Falsification: ถอด `try/catch` รอบ `this.redisCache.get` ใน `tenant.guard.ts` ชั่วคราว ยืนยันเทสต์ทั้งสองพัง (`expected 500 to be 200`) แล้ว revert
...
- [x] `owner` กด `POST /devices/{id}/retire` เครื่อง `pos` ที่มีกะเปิดอยู่ → กะถูกปิดใน transaction เดียวกัน, token เดิมของเครื่องนั้น refresh ไม่ผ่านภายใน 15 นาที, enrol เครื่องใหม่ได้ `device_no` ใหม่ และขายได้ (ADR-0004/0009) — **#384 2026-09-22** (`server/test/shifts.e2e-spec.ts:562` "a replacement pos device starts clean after the old one is retired") ปิดช่องว่างที่ untick ไว้ก่อนหน้านี้: เทสต์ตอนนี้แลก `enrolCode` ผ่าน `POST /auth/device` จริง (`:588`) ยืนยันว่าโค้ดใช้แล้วซ้ำไม่ได้ (`:595` → `401`) แล้วล็อกอินจริงผ่าน `POST /auth/token` (`:600`) แทนการมินต์ `accessToken({...})` เอง · เพิ่มยืนยัน stock ก่อน/หลังขายจริง (`:668`, 10 → 9) · "token เดิม refresh ไม่ผ่านภายใน 15 นาที" ยังอ้างอิงเทสต์เดิมที่ `server/test/devices.e2e-spec.ts:206` (ใช้เส้นทาง enrol จริงด้วย) ตามที่บันทึกไว้ตอน untick
