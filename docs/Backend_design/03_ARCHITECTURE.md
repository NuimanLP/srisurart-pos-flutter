...
- [x] สร้าง tenant ใหม่ด้วย `POST /platform/tenants` แล้วล็อกอิน+ขายได้จริงโดยไม่ต้องแตะ psql (ADR-0001) — **#196 2026-09-17** (`server/test/platform.e2e-spec.ts:426` "creates tenant, logs in as owner, creates product, enrols POS device, logs in on POS, opens shift, and sells" — full loop ผ่าน HTTP APIs ล้วนๆ ไม่แตะ psql)
...
- [ ] ดับ `redis-cache` แล้วร้านที่ `suspended` **ยังถูกปฏิเสธ** และร้านปกติ**ยังใช้งานได้** (ADR-0003 ข้อ 5: status ตกไปอ่าน Postgres ไม่ fail-open/closed) — 🔴 **untick 2026-09-22 หลังตรวจโค้ดจริง** (เคยติ๊กไว้ "#196 2026-09-17" อ้าง `server/test/tenant-scope.e2e-spec.ts:186`) เทสต์นั้นพิสูจน์คนละเรื่อง: มัน `vi.spyOn(cache,'get'/'set').mockRejectedValue(...)` = **mock เมธอด ไม่ใช่ cache ที่ล่มจริง** (จับเคส connect ค้าง/ready-check/retry ไม่ได้) และยิง **`/api/v1/tx4-probe`** (ซึ่งมี `TenantGuard` เหมือน controller จริง — จุดอ่อนไม่ได้อยู่ที่ route ผิด แต่อยู่ที่ AC ของ #294 ขอ `GET /products` + `POST /sales` **พร้อมยืนยันว่า stock ลดจริง** ซึ่ง probe ให้ไม่ได้) ไม่ใช่ `GET /products` + `POST /sales` **ที่ AC ของ #294 ระบุไว้เอง** (ADR-0003 §5 ไม่ได้ระบุ endpoint ใดเลย มันพูดแค่เรื่องอ่าน `tenants.status` จาก Postgres ตอน cache miss/Redis ล่ม โดยห้าม fail-open/fail-closed) · ของที่เทสต์นั้นพิสูจน์ได้จริงคือ "guard จับ exception จาก cache แล้วตกไปอ่าน Postgres ได้" ซึ่งมีค่าแต่ไม่ใช่ช่องนี้ · ปิดช่องว่างที่ **#383**
...
- [ ] `owner` กด `POST /devices/{id}/retire` เครื่อง `pos` ที่มีกะเปิดอยู่ → กะถูกปิดใน transaction เดียวกัน, token เดิมของเครื่องนั้น refresh ไม่ผ่านภายใน 15 นาที, enrol เครื่องใหม่ได้ `device_no` ใหม่ และขายได้ (ADR-0004/0009) — 🔴 **untick 2026-09-22 หลังตรวจโค้ดจริง** (เคยติ๊กไว้ "#196 2026-09-17" อ้าง `server/test/shifts.e2e-spec.ts`) เทสต์นั้น **ข้ามขั้น enrol**: มันสร้างเครื่องด้วย `POST /devices` แล้ว **มินต์ `accessToken({...deviceRole:'pos'})` เอง** (`shifts.e2e-spec.ts:575`) ไม่มี `POST /auth/device` สักบรรทัด และไม่เคยใช้ `enrolCode` ที่ API คืนมา — คอมเมนต์ในเทสต์เองยังอ้างช่อง DoD ข้อนี้อยู่ · ของที่เทสต์นั้นพิสูจน์ได้จริง (และยังจริงอยู่): retire ปิดกะใน tx เดียวกัน, token เดิมถูกปฏิเสธด้วย `DEVICE_ROLE_FORBIDDEN`, กะเก่าอยู่ในประวัติครบ, เครื่องใหม่ได้ `device_no` ใหม่และขายได้ · 🟢 อย่าเขียนซ้ำของที่มีแล้ว: ประโยค "token เดิม refresh ไม่ผ่านภายใน 15 นาที" **มีเทสต์ครอบอยู่จริง** ที่ `server/test/devices.e2e-spec.ts:206` ("enrol → device token → login … retirement locks the token out of login and refresh", refresh ได้ 401 ที่ :238-241) — เทสต์นั้น **ใช้เส้นทาง enrol จริงด้วย** ช่องว่างจึงอยู่เฉพาะที่ ลูกโซ่ retire → เครื่องแทนที่ → enrol → ขาย ใน `shifts.e2e-spec.ts` เท่านั้น · ปิดช่องว่างที่ **#384**
...
> **ยืนยันซ้ำ 2026-09-22 (#251 ปิดแล้ว):** มีการทบทวนว่าจะ**ยกเว้น IP เครื่องยิงออกจาก `perip`** ใน
> `nginx.conf` แทน เพื่อให้คนเดียวรันวัดได้ไม่ต้องนัดสามคน — **ปฏิเสธ** ด้วยเหตุผลสองข้อ: carve-out ที่
> เปิดค้างบนเครื่อง production เป็นความเสี่ยงถาวรแลกความสะดวกรอบเดียว และต่อให้ยกเว้นแล้ว "1 IP ยิงผ่าน
> edge ที่ปิดตัวจำกัดของตัวเอง" ก็ไกลจากรูปร่างทราฟฟิกจริงมากกว่า 3 IP จริง ⇒ ข้อความ
> **"ไม่มีข้อยกเว้นให้ `perip`"** ด้านล่างยังบังคับใช้ ห้ามเพิ่ม carve-out โดยไม่ถามเจ้าของ
> การ**ไปรัน**ให้เกิดตัวเลขจริงคือใบ **#380** (เลน C) — ปิด #251 แปลว่า "เคาะวิธีวัดแล้ว" ไม่ใช่ "วัดแล้ว"
> ช่อง DoD `k6 ผ่านเกณฑ์ใน 02 §9` ด้านบนจึงยังไม่ติ๊ก
