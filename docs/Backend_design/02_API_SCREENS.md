...
| 400 | `WEAK_PASSWORD` | `รหัสผ่านไม่ผ่านเกณฑ์ ต้องมีอย่างน้อย 12 ตัวอักษร` (#364, agent ร่าง — ops เห็นเท่านั้น ไม่ขึ้นที่หน้าร้าน) |
...
| 400 | `WEAK_PASSWORD` | `รหัสผ่านไม่ผ่านเกณฑ์ ต้องมีอย่างน้อย 12 ตัวอักษร` — agent ร่าง (#364, 2026-09-21) | `POST /platform/tenants` ที่ `ownerPassword` ว่าง หรือสั้นกว่า **12 ตัวอักษร** — เกณฑ์เดียวกับ `bootstrap:admin` (#337) จากฟังก์ชันเดียวกัน (`common/password.ts` → `passwordPolicyViolation` / `MIN_PASSWORD_LENGTH`) · ปฏิเสธ**ก่อน** hash argon2 และ**ก่อน**เปิดธุรกรรม จึงไม่มี tenant/owner ค้าง · **ไม่ขึ้นที่หน้าร้าน** — เห็นเฉพาะคนที่ provision ร้าน (`message` เป็นอังกฤษสำหรับ ops) จึงไม่ต้องรอเจ้าของร้านเคาะคำ |
