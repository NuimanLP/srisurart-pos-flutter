...
| Review Kind ที่ 6 `receipt_renumbered` (เพิ่ม 2026-09-25) | `เลขใบเสร็จออฟไลน์ไม่ตรงกับระบบ` — **agent ร่าง** รอเจ้าของโปรเจกต์รับรอง | push replay บิล/ใบลดหนี้ที่ commit ออนไลน์ไปแล้วด้วยเลขอื่น (คำตอบหาย แล้วเครื่องออกเลขออฟไลน์ใหม่) · `details { offlineNo, serverNo }` ให้ตามลูกค้าที่ถือใบเลขออฟไลน์ได้ · `frontend/lib/domain/models/review_item.dart` |
