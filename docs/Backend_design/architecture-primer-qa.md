...
>
> 🗓️ **หมายเหตุ 2026-09-23 — เอกสารนี้เป็นบันทึกประวัติ ไม่ได้แก้เนื้อหาการซักค้าน** แต่ข้อเท็จจริงบางข้อที่อ้างไว้เปลี่ยนไปแล้ว อย่าอ้างต่อโดยไม่ตรวจ:
> - **Q2:** "Nginx จำกัด Rate Limit ที่ 100 req/min ต่อร้านค้า" **ไม่เคยเป็นจริง** — Nginx จำกัด*ต่อ IP* (`perip` 30 r/s, burst 60) ส่วนการจำกัด*ต่อร้าน*อยู่ที่ `TenantRateLimitGuard` ใน NestJS (ADR-0006, ค่าเริ่มต้น 300 ครั้ง/60 วินาทีต่อร้านต่อ route) · ข้อสรุปเรื่อง Pool 62/100 ยังถูก
> - **Q3:** การตรวจ PIN ผู้จัดการก่อน void (`VoidService.authorise`, `tx.5`) **ถูกถอดออกแล้วในเฟส 2** — void ออนไลน์ใช้เหตุผลอย่างเดียว (08 E3) และ migration `…3001-SingleOwnerRole` ลบ `users.pin_hash` · หลัก "งานช้าอย่าง argon2 ต้องอยู่นอกทรานแซกชัน" ยังใช้กับการล็อกอิน
> - **Q1:** "Drift Schema v3" — วันนี้ Drift อยู่ที่ `schemaVersion` 11 · `offlineOk` ถูกยกเลิกในเฟส 2 (ADR-0004 D3) · เฟส 2 เริ่มลงมือแล้ว (`POST /sync/push` มีใน `server/src/sync/`)
> - [`architecture-primer.md`](architecture-primer.md) ถูกแก้ข้อเท็จจริงเพิ่มเมื่อ 2026-09-23 (ดูหมายเหตุ `🔄 แก้ 2026-09-23` ในไฟล์นั้น) — บันทึกนี้ตรวจฉบับก่อนหน้า
