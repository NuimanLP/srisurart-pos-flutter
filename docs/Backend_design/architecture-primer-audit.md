...
>
> 🗓️ **หมายเหตุ 2026-09-23 — บันทึกการตรวจนี้เป็นประวัติ ผลตรวจไม่ได้แก้** ผล PASS ข้างล่างใช้กับ `architecture-primer.md` ฉบับ 2026-09-21
> ซึ่งถูกแก้ข้อเท็จจริงเพิ่มเมื่อ 2026-09-23 (RLS 25 → 26 ตาราง, 27 → 29 ตาราง, void ไม่ใช้ PIN แล้ว, เลข RC/CN ออกที่เครื่อง `pos` ตาม ADR-0007 D4) — ยังไม่ได้ตรวจ A1–A15 ซ้ำกับฉบับใหม่
> - **A1:** เลขบรรทัดที่อ้าง (104/117/122/135/…) เลื่อนไปแล้ว ให้ค้นด้วยหัวข้อแทน
> - **A6:** ตัวเลขที่อ้างเปลี่ยนแล้ว — วันนี้มี unit spec **49 ไฟล์** (เดิม "397 passed / 47 test files") และ e2e **53 ไฟล์** (เดิม 51) · ตัวเลข `112ms → 18–28ms` ยังตรงกับคอมเมนต์ใน `test/tx-hold-measure.e2e-spec.ts` · Pool 62/100 ยังตรงกับ `server/docker-compose.yml`
