...
**7. กติกาการซิงก์ตอน Reconnect และ Sync Cursor — เพิ่ม 2026-09-15 (#191)**

เมื่อเครื่อง `pos` กลับมาต่อเน็ตหลังจากออฟไลน์:
* **ลำดับการซิงก์:** Push Outbox ขึ้นไปประมวลผลบน Server ให้เสร็จสิ้นทั้งหมดก่อน แล้วจึง Pull สต็อกลงมา
  (เพราะ Server เป็น Single Source of Truth เมื่อบิลขายถูกบันทึกลง Postgres แล้ว สต็อกบน Server จะถูกต้อง
  เครื่องจึง overwrite สต็อกใน Drift ด้วยค่าจาก Server ได้ตรง ๆ ทันที)
* **การป้องกัน Partial Push:** ในระหว่างที่ยังมีบิลขายค้างอยู่ใน Outbox (เช่น Push ไปได้บางส่วนแล้วเน็ตหลุด)
  ให้ระงับการ overwrite สต็อกของสินค้าที่มีรายการรอ push อยู่ในคิว เพื่อป้องกันตัวเลขสต็อกเด้งหลอกหน้าร้าน
* **`offlineOk` คำนวณที่ Client:** เครื่อง Client เป็นผู้ประเมิน `Products.offlineOk` เองจาก Threshold ในเครื่อง
  เพื่อตัดสิทธิ์ขายทันทีเมื่อยอดสต็อกในเครื่องลดลงต่ำกว่าเกณฑ์ระหว่างออฟไลน์
* **Sync Cursor & Read-back Window:** การดึงข้อมูลแบบ incremental ผ่าน `?updatedSince=` ใช้ keyset pagination
  `(updated_at, id)` ร่วมกับ **Read-back window ถอยหลัง 60 วินาที** จากเวลาที่ดึงล่าสุดเสมอ เพื่อป้องกันข้อมูลตกหล่น
  จาก Transaction ที่ commit ช้า และครอบคลุม Clock Skew 30 วินาที (ADR-0009); การเซฟลง Drift เป็น Idempotent
* **Tombstones สำหรับสินค้าที่ถูกลบ:** Endpoint ดึงข้อมูลต้องส่งสินค้าที่ `deleted_at IS NOT NULL` (Tombstone)
  ลงมาด้วย เพื่อให้เครื่องซ่อน/ลบสินค้านั้นออกจาก Drift ได้ถูกต้อง
...
* [x] ~~**(#55, เพิ่ม 2026-09-14 จาก #16) read-back window ของ `?updatedSince=`**~~ — **เคาะแล้ว 2026-09-15 (#191):**
      ใช้ keyset `(updated_at, id)` ร่วมกับ **read-back window ถอยหลัง 60 วินาที** ครอบคลุมทั้ง transaction ที่ commit ช้า
      และ clock skew (30s จาก ADR-0009); การเขียนลง Drift เป็น idempotent แถวซ้ำทับค่าเดิมได้โดยไม่มีผลข้างเคียง
...
* [x] ~~**ถามเจ้าของโปรเจกต์:** เมื่อ server รับบิลแล้ว แอปต้องเชื่อตัวเลขของ server และทับของในเครื่อง
      เสมอไหม แม้เครื่องจะเห็นต่าง~~ — **เคาะแล้ว 2026-09-15 (#191):** ใช่ เชื่อ server เป็น source of truth เสมอ
      ลำดับการ reconnect คือ push outbox ให้หมดก่อน แล้วจึง pull สต็อกมา overwrite ทับ Drift
