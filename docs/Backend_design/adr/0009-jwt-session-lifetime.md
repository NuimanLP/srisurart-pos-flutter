...
## 🔴 ข้อขัดแย้งที่ต้องแก้ก่อนเฟส 2 (เคาะแล้ว 2026-09-15 — #187)
...
* **เฟส 2 ข้อตัดสินใจและ Threat Model (#187):**
  1. **แคชเฉพาะแคชเชียร์ประจำเครื่อง `pos`:** **ห้ามแคช PIN ของ `owner` เด็ดขาด** เพื่อตัดความเสี่ยง Privilege Escalation จากการเจาะแคชในเบราว์เซอร์
  2. **Device-bound Credential + Local Throttling:** ไม่ก๊อป `pin_hash` จาก server มาตรง ๆ แต่แฮช PIN ด้วย salt เฉพาะของเครื่อง (`deviceToken` secret) ทำให้แม้แฮชใน IndexedDB หลุด ก็เดา PIN บน server ไม่ได้โดยตรง พร้อมระบบจำกัดกรอกผิดในแอป (เช่น ผิดเกิน 5 ครั้งล็อกหน่วงเวลาแบบ exponential backoff)
  3. **สิทธิ์ของกะที่เปิดแบบออฟไลน์:** ขายได้เฉพาะสินค้า `offlineOk` เท่านั้น, **ห้าม Void บิลเด็ดขาด** (void ต้องรอเน็ตเพื่อตรวจ manager PIN และย้อน stock/ledger), ปิดลิ้นชักทางกายภาพได้ แต่ยอดสรุปและใบปิดกะทางการจะยืนยันเมื่อต่อเน็ตและ sync สำเร็จ
  4. **อายุของ Offline Session:** สิ้นสุดเวลา **04:00 ของวันถัดไป** (ตาม `tenants.timezone`) และทันทีที่ต่อเน็ตได้ แอปต้องแลกเป็น Token จริงในเบื้องหลังทันที
...
* [x] ~~เฟส 2: refresh ที่นอนอยู่ใน IndexedDB ของเครื่อง `pos` ตลอดคืน ควรผูกกับ device token
      (`refresh` ใช้ได้เฉพาะเมื่อส่งคู่กับ `deviceToken` ตัวเดิม) ไหม~~ — **เคาะแล้ว 2026-09-15 (#187):**
      ผูกเสมอ — `/auth/refresh` บนเครื่อง `pos` ต้องส่งคู่กับ `deviceToken` ตัวเดิม
