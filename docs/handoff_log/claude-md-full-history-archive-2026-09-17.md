# CLAUDE.md full history archive (backend/CI/CD/phase-2 build log, through 2026-09-17)

This is the verbatim "Migration status" section of `CLAUDE.md` as it stood before the
2026-09-17 trim. It is the full PR-by-PR, ticket-by-ticket narrative of the backend +
CI/CD + multi-tenant client build — every merged PR, every bug found and fixed, every
owner decision, in chronological order. `CLAUDE.md` itself now carries only a condensed
summary plus the rules from this log that are still binding; this file is where you look
when you need the *why* behind one of those rules, or the story of a ticket the condensed
version only names in passing.

Nothing below was edited or summarized — it is a direct extract, kept for the record.
